...
ABI hash. Two uploads that share all four are the same package, so harbor uses
that 4-tuple as both the storage key and the deduplication key.
...
- Because the identity implies the content, an upload of *different* bytes under
  an identity that already exists keeps the stored copy.
...
- Reference counter updates are serialised per package inside one worker process.
  Two workers registering the same package in different tags at the same instant
  can race, which may leave a reference count that is one too low. Deleting a tag
  can then remove bytes another tag references; the other tag's next request is
  a cache miss and re-uploads them.
